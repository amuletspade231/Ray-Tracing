    ...
    const vec3& normal,int recursion_depth) const
{
	...
	if (shadow_hit.object == NULL || shadow_hit.dist > light_vec.magnitude()) {
	    diffuse += color_diffuse * light->Emitted_Light(light_vec) * std::max(dot(normal, shadow_ray.direction), 0.0);
	    
	    vec3 reflection = (ray.direction - 2 * dot(ray.direction, normal) * normal).normalized();
	    vec3 view = - ray.direction;
	    specular += color_specular * light->Emitted_Light(light_vec) * pow(std::max(dot(view, reflection) ,0.0), specular_power); 
	}
    }
